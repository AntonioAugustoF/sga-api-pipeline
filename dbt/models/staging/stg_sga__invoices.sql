-- One row per invoice in the most recent extraction batch, with the rules from
...
with latest_batch as (
    ...
    from {{ source('sga', 'invoices') }}
    where _extracted_at = (select max(_extracted_at) from {{ source('sga', 'invoices') }})
